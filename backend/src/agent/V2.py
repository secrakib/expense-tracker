...
def update_extract_node(state: AgentState) -> AgentState:
    ...
    params: UpdateParams = extractor.invoke([
        SYSTEM_PROMPT,
        HumanMessage(content=
            f"Extract update parameters (id, category, expense amount, date).\n"
            f"User said: {user_msg}"
        )
    ])
    matches = fuzzy_match(params.category, cats) if params.category else []
...
def update_fields_node(state: AgentState) -> AgentState:
    """Ask for what to change if nothing specified. One interrupt."""
    cat     = state.get("resolved_category")
    expense = state.get("stage_expense")
    date    = state.get("stage_date")
    ...
    refill: UpdateParams = extractor.invoke([
        SYSTEM_PROMPT,
        HumanMessage(content=f"Extract fields to update (category, expense, date) from: {answer}")
    ])
...
def build_graph():
    ...
    g.add_node("add_fuzzy",     fuzzy_node)          # dedicated copy for add flow
    ...
    g.add_edge("fuzzy",        "view_execute")
    g.add_edge("view_execute", END)

    # ── add flow ──────────────────────────────────────────────────────────────
    # Note: we reuse the fuzzy_node function but register it as "add_fuzzy"
    # so each flow has its own node instance and edges don't collide.
    ...
    g.add_edge("add_confirm",  "confirm")
    g.add_edge("confirm",      "add_execute")
    g.add_edge("add_execute",  END)

    # ── update flow ───────────────────────────────────────────────────────────
    g.add_conditional_edges("update_extract", update_id_done, {
        "done":  "update_id",      # id already known, skip to fuzzy check
        "retry": "update_id",
        "abort": END,
    })
    # Re-check after update_id node
    ...
    # Note: after fuzzy resolves, go to fields
    # We need a direct edge from update_fuzzy (the node) to update_fields.
    # Since add_conditional_edges above handles it, add the fuzzy→fields edge:
    g.add_edge("update_fuzzy", "update_fields")  # will be overridden only if fuzzy needed
    g.add_conditional_edges("update_fields", update_fields_done, {
        "done":  "update_confirm",
        "retry": "update_fields",
        "abort": END,
    })
    g.add_edge("update_confirm", "confirm")
    g.add_edge("confirm",        "update_execute")   # shared confirm must go somewhere...
    ...
    g.add_edge("delete_confirm", "confirm")
    g.add_edge("confirm",        "delete_execute")
...
def main(username: str):
    ...
    while True:
        # ── check for a pending interrupt before asking for new input ─────────
        snapshot = graph.get_state(config)
        if snapshot and snapshot.next:
            # Graph is paused mid-flow — resume it
            try:
            ...
            for _ in graph.stream(Command(resume=user_input), config=config, stream_mode="values"):
                pass
            continue  # loop back to check if another interrupt is pending

        # ── no pending interrupt — get a fresh request ────────────────────────
        try:
            user_input = input("You: ").strip()
        except (EOFError, KeyboardInterrupt):
            break
